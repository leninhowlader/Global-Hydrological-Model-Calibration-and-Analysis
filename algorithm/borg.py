...
class BorgConfiguration:
    ...
    def check():
        ...
        try:
            BorgConfiguration.libc
        except:
            raise OSError("The standard C library is not defined, please see BorgConfiguration.setStandardCLibrary(<file>)")

        try:
            BorgConfiguration.libborg
        except:
            raise OSError("The Borg MOEA C library is not defined, please see BorgConfiguration.setBorgLibrary(<file>)")

    @staticmethod
    def initialize():
        """ Initializes the standard C and Borg MOEA libraries. """
        BorgConfiguration.setStandardCLibrary()
        BorgConfiguration.setBorgLibrary()
        BorgConfiguration.seed()
        BorgConfiguration.startedMPI = False

    @staticmethod
    def getWorldRank():
        return BorgConfiguration.libborg.BORG_WorldRank()

    @staticmethod
    def getWorldSize():
        return BorgConfiguration.libborg.BORG_WorldSize()
    ...
    def setStandardCLibrary(path=None):
        ...
        if path:
            BorgConfiguration.libc = CDLL(path)
        elif os.name == "posix":
            try:
                BorgConfiguration.libc = CDLL("libc.so.6")
            except OSError:
                return
        elif os.name == "nt" and cdll.msvcrt:
            BorgConfiguration.libc = cdll.msvcrt
        else:
            return

        try:
            BorgConfiguration.stdout = BorgConfiguration.libc.fdopen(sys.stdout.fileno(), "w")
        except AttributeError:
            BorgConfiguration.stdout = BorgConfiguration.libc._fdopen(sys.stdout.fileno(), "w")
    ...
    def setBorgLibrary(path=None):
        ...
        if path:
            try:
                BorgConfiguration.libborg = CDLL(path)
                BorgConfiguration.libborg.BORG_Copyright
                BorgConfiguration.stdcall = False
            except AttributeError:
                # Not using __cdecl, try __stdcall instead
                if os.name == "nt":
                    BorgConfiguration.libborg = WinDLL(path)
                    BorgConfiguration.stdcall = True
        elif os.name == "posix":
            try:
                BorgConfiguration.libborg = CDLL("./libborg.so")
                BorgConfiguration.stdcall = False
            except OSError:
                return
        elif os.name == "nt":
            try:
                BorgConfiguration.libborg = CDLL("./borg.dll")
                BorgConfiguration.libborg.BORG_Copyright
                BorgConfiguration.stdcall = False
            except OSError:
                return
                ...
                # Not using __cdecl, try __stdcall instead
                try:
                    BorgConfiguration.libborg = WinDLL("./borg.dll")
                    BorgConfiguration.stdcall = True
                except OSError:
                    return
                    
        # Set result type of functions with non-standard types
        BorgConfiguration.libborg.BORG_Problem_create.restype = c_void_p
        BorgConfiguration.libborg.BORG_Operator_create.restype = c_void_p
        BorgConfiguration.libborg.BORG_Algorithm_create.restype = c_void_p
        BorgConfiguration.libborg.BORG_Algorithm_get_result.restype = c_void_p
        BorgConfiguration.libborg.BORG_Archive_get.restype = c_void_p
        BorgConfiguration.libborg.BORG_Solution_get_variable.restype = c_double
        BorgConfiguration.libborg.BORG_Solution_get_objective.restype = c_double
        BorgConfiguration.libborg.BORG_Solution_get_constraint.restype = c_double
        BorgConfiguration.libborg.BORG_Operator_get_probability.restype = c_double
    ...
    def seed(value=None):
        ...
        if value:
            BorgConfiguration.libborg.BORG_Random_seed(c_ulong(value))
        else:
            BorgConfiguration.libborg.BORG_Random_seed(c_ulong(os.getpid()*long(time.time())))

    @staticmethod
    def enableDebugging():
        """ Enables debugging output from the Borg MOEA. """
        BorgConfiguration.check()
        BorgConfiguration.libborg.BORG_Debug_on()

    @staticmethod
    def disableDebugging():
        """ Disables debugging output from the Borg MOEA. """
        BorgConfiguration.check()
        BorgConfiguration.libborg.BORG_Debug_off()

    @staticmethod
    def displayCopyright():
        """ Displays the copyright message for the Borg MOEA. """
        BorgConfiguration.check()
        BorgConfiguration.libborg.BORG_Copyright(BorgConfiguration.stdout)

    @staticmethod
    def startMPI(libmpi_path:str=''):
        """ Initializes MPI to enable master-slave and multi-master Borg MOEA runs. """
        if BorgConfiguration.startedMPI:
        ...
        try:
            BorgConfiguration.libborg.BORG_Algorithm_ms_startup
        except AttributeError:
            # The serial Borg MOEA C library is loaded; switch to parallel
            try:
                BorgConfiguration.setBorgLibrary("./libborgmm.so")
            except OSError:
                try:
                    BorgConfiguration.setBorgLibrary("./libborgms.so")
                except OSError:
        ...
        BorgConfiguration.startedMPI = True
    ...
    def stopMPI():
        ...
        BorgConfiguration.libborg.BORG_Algorithm_ms_shutdown()
        BorgConfiguration.startedMPI = False
...
class Borg:
    ...
    def _setEpsilon(self, index, value):
        ...
        BorgConfiguration.libborg.BORG_Problem_set_epsilon(self.reference, index, c_double(value))
    ...
    def solveMPI(
        ...
        if islands > 1:
            try:
                BorgConfiguration.libborg.BORG_Algorithm_ms_islands(c_int(islands))
            except AttributeError:
                raise RuntimeError("The loaded Borg MOEA C library does not support multi-master")

        if maxTime:
            BorgConfiguration.libborg.BORG_Algorithm_ms_max_time(c_double(maxTime))

        if maxEvaluations:
            BorgConfiguration.libborg.BORG_Algorithm_ms_max_evaluations(c_int(maxEvaluations))

        if initialization and islands > 1:
            BorgConfiguration.libborg.BORG_Algorithm_ms_initialization(c_int(initialization));

        if runtime:
            BorgConfiguration.libborg.BORG_Algorithm_output_runtime(
                c_char_p(runtime.encode('utf-8'))
            );
    ...
    def solve(self, settings={}):
        ...
        pm = c_void_p(BorgConfiguration.libborg.BORG_Operator_create("PM", 1, 1, 2, BorgConfiguration.libborg.BORG_Operator_PM))
        BorgConfiguration.libborg.BORG_Operator_set_parameter(pm, 0, c_double(settings.get("pm.rate", 1.0 / self.numberOfVariables)))
        BorgConfiguration.libborg.BORG_Operator_set_parameter(pm, 1, c_double(settings.get("pm.distributionIndex", 20.0)))
        
        sbx = c_void_p(BorgConfiguration.libborg.BORG_Operator_create("SBX", 2, 2, 2, BorgConfiguration.libborg.BORG_Operator_SBX))
        BorgConfiguration.libborg.BORG_Operator_set_parameter(sbx, 0, c_double(settings.get("sbx.rate", 1.0)))
        BorgConfiguration.libborg.BORG_Operator_set_parameter(sbx, 1, c_double(settings.get("sbx.distributionIndex", 15.0)))
        BorgConfiguration.libborg.BORG_Operator_set_mutation(sbx, pm)

        de = c_void_p(BorgConfiguration.libborg.BORG_Operator_create("DE", 4, 1, 2, BorgConfiguration.libborg.BORG_Operator_DE))
        BorgConfiguration.libborg.BORG_Operator_set_parameter(de, 0, c_double(settings.get("de.crossoverRate", 0.1)))
        BorgConfiguration.libborg.BORG_Operator_set_parameter(de, 1, c_double(settings.get("de.stepSize", 0.5)))
        BorgConfiguration.libborg.BORG_Operator_set_mutation(de, pm)

        um = c_void_p(BorgConfiguration.libborg.BORG_Operator_create("UM", 1, 1, 1, BorgConfiguration.libborg.BORG_Operator_UM))
        BorgConfiguration.libborg.BORG_Operator_set_parameter(um, 0, c_double(settings.get("um.rate", 1.0 / self.numberOfVariables)))

        spx = c_void_p(BorgConfiguration.libborg.BORG_Operator_create("SPX", c_int(settings.get("spx.parents", 10)), c_int(settings.get("spx.offspring", 2)), 1, BorgConfiguration.libborg.BORG_Operator_SPX))
        BorgConfiguration.libborg.BORG_Operator_set_parameter(spx, 0, c_double(settings.get("spx.epsilon", 3.0)))

        pcx = c_void_p(BorgConfiguration.libborg.BORG_Operator_create("PCX", c_int(settings.get("pcx.parents", 10)), c_int(settings.get("pcx.offspring", 2)), 2, BorgConfiguration.libborg.BORG_Operator_PCX))
        BorgConfiguration.libborg.BORG_Operator_set_parameter(pcx, 0, c_double(settings.get("pcx.eta", 0.1)))
        BorgConfiguration.libborg.BORG_Operator_set_parameter(pcx, 1, c_double(settings.get("pcx.zeta", 0.1)))

        undx = c_void_p(BorgConfiguration.libborg.BORG_Operator_create("UNDX", c_int(settings.get("undx.parents", 10)), c_int(settings.get("undx.offspring", 2)), 2, BorgConfiguration.libborg.BORG_Operator_UNDX))
        BorgConfiguration.libborg.BORG_Operator_set_parameter(undx, 0, c_double(settings.get("undx.zeta", 0.5)))
        BorgConfiguration.libborg.BORG_Operator_set_parameter(undx, 1, c_double(settings.get("undx.eta", 0.35)))

        algorithm = c_void_p(BorgConfiguration.libborg.BORG_Algorithm_create(self.reference, 6))
        BorgConfiguration.libborg.BORG_Algorithm_set_operator(algorithm, 0, sbx)
        BorgConfiguration.libborg.BORG_Algorithm_set_operator(algorithm, 1, de)
        BorgConfiguration.libborg.BORG_Algorithm_set_operator(algorithm, 2, pcx)
        BorgConfiguration.libborg.BORG_Algorithm_set_operator(algorithm, 3, spx)
        BorgConfiguration.libborg.BORG_Algorithm_set_operator(algorithm, 4, undx)
        BorgConfiguration.libborg.BORG_Algorithm_set_operator(algorithm, 5, um)

        BorgConfiguration.libborg.BORG_Algorithm_set_initial_population_size(algorithm, c_int(settings.get("initialPopulationSize", 100)))
        BorgConfiguration.libborg.BORG_Algorithm_set_minimum_population_size(algorithm, c_int(settings.get("minimumPopulationSize", 100)))
        BorgConfiguration.libborg.BORG_Algorithm_set_maximum_population_size(algorithm, c_int(settings.get("maximumPopulationSize", 10000)))
        BorgConfiguration.libborg.BORG_Algorithm_set_population_ratio(algorithm, c_double(1.0 / settings.get("injectionRate", 0.25)))
        BorgConfiguration.libborg.BORG_Algorithm_set_selection_ratio(algorithm, c_double(settings.get("selectionRatio", 0.02)))
        BorgConfiguration.libborg.BORG_Algorithm_set_restart_mode(algorithm, c_int(settings.get("restartMode", RestartMode.DEFAULT)))
        BorgConfiguration.libborg.BORG_Algorithm_set_max_mutation_index(algorithm, c_int(settings.get("maxMutationIndex", 10)))
        BorgConfiguration.libborg.BORG_Algorithm_set_probability_mode(algorithm, c_int(settings.get("probabilityMode", ProbabilityMode.DEFAULT)))
        ...
        while BorgConfiguration.libborg.BORG_Algorithm_get_nfe(algorithm) < maxEvaluations:
            BorgConfiguration.libborg.BORG_Algorithm_step(algorithm)
            if terminate is True:
                    break
            currentEvaluations = BorgConfiguration.libborg.BORG_Algorithm_get_nfe(algorithm)
            ...
            if statistics is not None and currentEvaluations-lastSnapshot >= frequency:
                ...
                entry["SBX"] = BorgConfiguration.libborg.BORG_Operator_get_probability(sbx)
                entry["DE"] = BorgConfiguration.libborg.BORG_Operator_get_probability(de)
                entry["PCX"] = BorgConfiguration.libborg.BORG_Operator_get_probability(pcx)
                entry["SPX"] = BorgConfiguration.libborg.BORG_Operator_get_probability(spx)
                entry["UNDX"] = BorgConfiguration.libborg.BORG_Operator_get_probability(undx)
                entry["UM"] = BorgConfiguration.libborg.BORG_Operator_get_probability(um)
                entry["Improvements"] = BorgConfiguration.libborg.BORG_Algorithm_get_number_improvements(algorithm)
                entry["Restarts"] = BorgConfiguration.libborg.BORG_Algorithm_get_number_restarts(algorithm)
                entry["PopulationSize"] = BorgConfiguration.libborg.BORG_Algorithm_get_population_size(algorithm)
                entry["ArchiveSize"] = BorgConfiguration.libborg.BORG_Algorithm_get_archive_size(algorithm)

                if settings.get("restartMode", RestartMode.DEFAULT) == RestartMode.ADAPTIVE:
                    entry["MutationIndex"] = BorgConfiguration.libborg.BORG_Algorithm_get_mutation_index(algorithm)
                if fp is None:
                    statistics.append(entry)
                else:
                    archive = Result(c_void_p(BorgConfiguration.libborg.BORG_Algorithm_get_result(algorithm)), self, statistics)
                    if runtimeformat == 'optimizedv':
        ...
        result = c_void_p(BorgConfiguration.libborg.BORG_Algorithm_get_result(algorithm))

        if fp is not None:
            fp.close()

        BorgConfiguration.libborg.BORG_Operator_destroy(sbx)
        BorgConfiguration.libborg.BORG_Operator_destroy(de)
        BorgConfiguration.libborg.BORG_Operator_destroy(pm)
        BorgConfiguration.libborg.BORG_Operator_destroy(um)
        BorgConfiguration.libborg.BORG_Operator_destroy(spx)
        BorgConfiguration.libborg.BORG_Operator_destroy(pcx)
        BorgConfiguration.libborg.BORG_Operator_destroy(undx)
        BorgConfiguration.libborg.BORG_Algorithm_destroy(algorithm)
...
class Solution:
    ...
    def _getVariable(self, index):
        ...
        return BorgConfiguration.libborg.BORG_Solution_get_variable(self.reference, index)

    def _getObjective(self, index):
        """ Returns the objective value at the given index. """
        value = BorgConfiguration.libborg.BORG_Solution_get_objective(self.reference, index)
    ...
    def violatesConstraints(self):
        ...
        return BorgConfiguration.libborg.BORG_Solution_violates_constraints(self.reference) != 0
...
class Result:
    ...
    def __del__(self):
        """ Deletes the underlying C objects. """
        BorgConfiguration.libborg.BORG_Archive_destroy(self.reference)
    ...
    def size(self):
        ...
        return BorgConfiguration.libborg.BORG_Archive_get_size(self.reference)

    def get(self, index):
        """ Returns the Pareto optimal solution at the given index. """
        return Solution(c_void_p(BorgConfiguration.libborg.BORG_Archive_get(self.reference, index)), self.problem)
